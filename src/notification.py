...
def send_discord_notify(job_data, ai_result):
    ...
    embed = {
        ...
        "color": color,
        "fields": [
            {"name": "👍 メリット", "value": ai_result.get("pros", "なし"), "inline": False},
            {"name": "👎 懸念点", "value": ai_result.get("cons", "なし"), "inline": False},
            {"name": "📝 AIコメント", "value": ai_result.get("reason", "なし"), "inline": False}
        ],
        "footer": {"text": f"JobHunter-Bot | {job_data['subject']}"}
    }

    payload = {
        "username": "AI就活コンシェルジュ",
        "embeds": [embed]
    }
